...
if ($?_complete) then
    ...
    unset f
    if ( ! $?hosts ) then
	set hosts=({cypher,angel,sunfire,magma}.strobe.net)
    endif
    ...
    complete mutt	"n@-f@f@" \
			...
			n/-s/x:'<subject line>'/ \
			n/-e/x:'<command>'/ \
			n@-b@'`cat "${HOME}/.mutt-alias" | awk '"'"'{print $2 }'"'"\`@ \
			n@-c@'`cat "${HOME}/.mutt-alias" | awk '"'"'{print $2 }'"'"\`@ \
			n@*@'`cat "${HOME}/.mutt-alias" | awk '"'"'{print $2 }'"'"\`@
